...
Then(/^I should see urls for the following cached thumbnails:$/) do |table|
  table.hashes.each do |row|
    ...
    if(row['type'] == "img")
      imagedef = "<img src=\"#{urlPrefix}images/original.jpg?simple-thumbnailer=original.jpg%7C#{URI.escape(row['size'])}\" class=\"image-resized-to#{row['class']}\" alt=\"Original.jpg?simple thumbnailer=original\" />"
    else
      imagedef = "<source srcset=\"#{urlPrefix}images/original.jpg?simple-thumbnailer=original.jpg%7C#{URI.escape(row['size'])}\" media=\"(min-width: 900px)\">"
    end
